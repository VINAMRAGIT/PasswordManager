...
public class DecryptData extends EncryptDecrypt
{
    ...
    BadPaddingException
    {
        ...
        if (DATA_FILE.length() != 0) {
            ...
            //Parse and store data in List
            while (decrypted.contains(">")) {
                String parse = decrypted.substring(1,
                                                   decrypted.indexOf('>'));
                System.out.println("TO PARSE: " + parse);

                goal.add(new PassEntry(parse.substring(0,
                                                       parse.indexOf(':')),
                                       parse.substring(parse.indexOf(':') + 1,
                                                       parse.lastIndexOf(':')),
                                       parse.substring(parse.lastIndexOf(':') + 1)));
                if (decrypted.indexOf(">") == decrypted.length() - 1)
                    break;
                else
